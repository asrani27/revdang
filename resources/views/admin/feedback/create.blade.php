...
<div class="p-6">
    ...
    <nav class="mb-4">
        <ol class="flex items-center gap-2 text-sm">
            ...
            <li class="text-slate-900 dark:text-white font-medium">Tambah Feedback</li>
        </ol>
    </nav>

    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Tambah Feedback Baru</h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Tambahkan data feedback pelanggan</p>
    </div>

    <!-- Form -->
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        <form action="{{ route('admin.data.feedback.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    @enderror
                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Hanya menampilkan pengaduan yang belum memiliki feedback</p>
                </div>
            ...
            <div class="mt-6">
                ...
                <textarea id="komentar" name="komentar" rows="4" 
                    ...
                    placeholder="Masukkan komentar feedback">{{ old('komentar') }}</textarea>
                @error('komentar')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
