...
<div class="p-6">
    ...
    <nav class="mb-4">
        <ol class="flex items-center gap-2 text-sm">
            ...
            <li class="text-slate-900 dark:text-white font-medium">Edit Gangguan</li>
        </ol>
    </nav>

    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit Gangguan</h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Perbarui data gangguan</p>
    </div>

    <!-- Form -->
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        <form action="{{ route('admin.data.gangguan.update', $gangguan->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="nama_gangguan" name="nama_gangguan" value="{{ old('nama_gangguan', $gangguan->nama_gangguan) }}" 
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4" 
                        ...
                        placeholder="Masukkan deskripsi gangguan">{{ old('deskripsi', $gangguan->deskripsi) }}</textarea>
                    @error('deskripsi')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
