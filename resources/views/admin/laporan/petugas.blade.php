...
<div class="p-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Laporan Petugas</h1>
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Data petugas dalam sistem</p>
        </div>
        <a href="{{ route('admin.laporan.petugas.pdf') }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors font-medium text-sm">
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                        ...
                        <td class="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">{{ $p->jabatan }}</td>
                        <td class="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">{{ $p->telp ?? '-' }}</td>
                        <td class="px-4 py-3">
                            <span class="inline-flex px-2.5 py-1 text-xs font-medium rounded-full {{ $p->user && $p->user->status_akun === 'aktif' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' }}">
                                {{ $p->user ? ucfirst($p->user->status_akun) : 'Tidak Ada Akun' }}
                            </span>
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="px-4 py-8 text-center text-slate-500 dark:text-slate-400">Belum ada data petugas</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
