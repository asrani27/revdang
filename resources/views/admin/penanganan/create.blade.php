...
<div class="p-6">
    ...
    <nav class="mb-4">
        <ol class="flex items-center gap-2 text-sm">
            ...
            <li class="text-slate-900 dark:text-white font-medium">Tambah Penanganan</li>
        </ol>
    </nav>

    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Tambah Penanganan Baru</h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Tambahkan data penanganan pengaduan</p>
    </div>

    <!-- Form -->
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        <form action="{{ route('admin.data.penanganan.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="pengaduan_id" name="pengaduan_id" 
                        ...
                        <option value="{{ $p->id }}" {{ old('pengaduan_id') == $p->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="petugas_id" name="petugas_id" 
                        ...
                        <option value="{{ $pt->id }}" {{ old('petugas_id') == $pt->id ? 'selected' : '' }}>
            ...
            <div class="mt-6">
                ...
                <textarea id="tindakan" name="tindakan" rows="4" 
                    ...
                    placeholder="Jelaskan tindakan yang dilakukan" required>{{ old('tindakan') }}</textarea>
                @error('tindakan')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea id="hasil" name="hasil" rows="4" 
                    ...
                    placeholder="Jelaskan hasil penanganan" required>{{ old('hasil') }}</textarea>
                @error('hasil')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
