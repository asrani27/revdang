...
<div class="p-6">
    ...
    <nav class="mb-4">
        <ol class="flex items-center gap-2 text-sm">
            ...
            <li class="text-slate-900 dark:text-white font-medium">Tambah Pengaduan</li>
        </ol>
    </nav>

    <!-- Header -->
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Tambah Pengaduan Baru</h1>
        <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Tambahkan data pengaduan pelanggan</p>
    </div>

    <!-- Form -->
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
        <form action="{{ route('admin.data.pengaduan.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="gangguan_id" name="gangguan_id" 
                        ...
                        <option value="{{ $g->id }}" {{ old('gangguan_id') == $g->id ? 'selected' : '' }}>
                            {{ $g->nama_gangguan }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="mt-6">
                ...
                <textarea id="keluhan" name="keluhan" rows="4" 
                    ...
                    placeholder="Jelaskan keluhan pelanggan" required>{{ old('keluhan') }}</textarea>
                @error('keluhan')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
